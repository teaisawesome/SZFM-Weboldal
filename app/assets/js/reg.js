...
//bankkartyaszam input checkbox 

const bankszam = document.querySelector('#bankszam');
const szovegBox = document.querySelectorAll('.elofizet');

var checkBox = document.getElementById('elofizetek');
var checkBoxx = document.getElementById('nemfizet');

bankszam.style.display = "none";

checkBox.addEventListener('click', function() {

    bankszam.style.display = "block";

    const tl = new TimelineMax({ delay: 0 });

    tl.fromTo(szovegBox, 0.4, { y: "50" }, { y: "0" })
        .fromTo(bankszam, 0.7, { x: '-350%' }, { x: '0%' });
